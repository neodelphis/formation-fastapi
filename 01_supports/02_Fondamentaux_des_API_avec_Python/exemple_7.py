...
# Compteur d'identifiants indépendant de la taille de la liste (évite les doublons après suppression)
NEXT_PLAYER_ID = max(player["id"] for player in PLAYERS) + 1
print(f"Prochain identifiant disponible : {NEXT_PLAYER_ID}")
...
def create_player(player: Player):
    ...
    # 2. On génère un nouvel ID à partir du compteur, puis on l'incrémente
    new_player["id"] = NEXT_PLAYER_ID
    NEXT_PLAYER_ID += 1
    print(f"Nouveau joueur créé avec l'identifiant {new_player['id']}, prochain identifiant : {NEXT_PLAYER_ID}")
